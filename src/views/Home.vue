  ...
  <div class="home">
        ...
        <div v-if="dataList.length">
          ...
          <div class="list-wrapper" v-for="(item,i) in dataList" :key="i" @click="detailright(item)">
            ...
            <div class="phone" v-if="item.linkTelarr1!==undefined&&query.orgType!==3">
              <p  v-for="(items,i) in item.linkTelarr1" :key="i"><van-icon name="phone-o" size="20" /><span>{{item.linkPeoplearr1[i]}}</span><span @click="searchRightModelPhone(items)">{{items}}</span></p>
            </div>
          </div>
          </div>
        <div v-else>
    ...
    <van-popup v-model="agreement" closeable position="bottom" :style="{ height: '100%' }">
      <div class="agreement-content">
        ...
        <div class="contentfont">4.5 本平台唯一官方网址：http://rescue.sisiits.com:9966/visur（新冠肺炎物资公益平台），如被第三方恶意篡改或假借名义进行不正当行为，与本平台无关，本平台不对用户或任何第三方受到的损害负责，但本平台保留对恶意第三方追究法律责任的权利。</div>
